...
class EditUsers extends Component {
	constructor() {
		...
		this.state = {
			titlePage: "Добавление оператора",
	...
	fillUser = id => {
		let error = "Ошибка при получении оператора!";
		postRequest("get-operators", {id})
			.then(res => {
				...
				this.setState({
					...
					titlePage: "Редактирование оператора",
				});
				store.dispatch(setIsShowPreloader(false));
			})
			.catch(err => {
	...
	addUser = () => {
		if (
			...
			store.dispatch(setError('Поля "Имя", "Фамилия", "Email", "Пароль", "Роль" обязательны для заполнения!'));
			return;
		}
		...
		postRequest("add-operators", newDataUser)
			.then(res => {
				if (res.success) {
					store.dispatch(setMessage("Оператор сохранен"));
					...
					<Grid container spacing={2}>
						...
						<Grid item xs={12} sm={this.state.id > 0 ? 8 : 12}>
							<Paper className={classes.paper}>
								...
								<div className={classes.blockInput}>
									<label className={classes.blockLabel}>Роль*</label>
									<Select
										className={classes.marginField}
										name="roleId"
										onChange={this.onChange}
										value={this.state.roleId}>
										<MenuItem value="100">Оператор тикетов</MenuItem>
										<MenuItem value="101">Оператор выплат</MenuItem>
									</Select>
								</div>
